...
- Nmap: Ports 22(SSH), 80(HTTP/CCTV), 502(Modbus)
- CCTV: Eggs on conveyor (HR0=1), "Compromised"
- PyModbus: HR0=1(Eggs), HR4=666(Eggsploit), C11=True(Protection), C10/13=False

**Fix Order (CRITICAL):**

1. Disable C11 protection
2. Set HR0=0 (Christmas)
3. Enable C10/13 (verification/logging)

<aside>
💡

- The material supplies us with a procedure to execute on the system in order to restore the settings to their original state and obtain the flag.
- Utilize the script provided below, but it is necessary to substitute the IP address in the `PLC_IP`variable.
</aside>
